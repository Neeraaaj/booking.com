...
export async function fetchResults(searchParams: SearchParams) {
  ...
  if (!username || !password) {
    console.error("Oxylabs credentials are not set.");
    return null;
  }
  ...
  const body = {
    source: "google",  // 'universal' is causing issues; try other sources if needed
    // url: `${url.href}`,
  ...
  try {
    ...
    const data: OxylabsResponse = await response.json() as OxylabsResponse;
